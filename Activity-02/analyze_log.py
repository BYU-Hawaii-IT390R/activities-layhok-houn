...
def analyze_failed_logins(path: str, min_count: int):
    ...
    # Filter out entries below the min_count
    filtered = Counter({ip: count for ip, count in counter.items() if count >= min_count})

    # Print the result
    print("Failed login attempts (min count: {})".format(min_count))
...
def analyze_successful_creds(path: str):
    ...
    print("Successful login credentials used by unique IPs")
...
# regex assisted by ChatGPT: match commands executed via shell
COMMAND_PATTERN = re.compile(r'CMD: (.+)')

def analyze_top_commands(path: str, top_n: int = 10):
    """Show the most frequent shell commands executed by attackers."""
    cmd_counter = Counter()
    ...
    print("-" * 50)
    for cmd, count in cmd_counter.most_common(top_n):
